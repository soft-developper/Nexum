...
function PayContent() {
  ...
  async function handlePay() {
    ...
    try {
      ...
      // Route the payment THROUGH the vault so the 0.1% platform fee is split
      // on-chain: the vault keeps the fee and forwards (amount - fee) to the
      // creator. Two steps for an external wallet: approve the vault to pull
      // USDC, then payInvoice. The invoice is reconciled by its DB memo_ref;
      // on-chain we key it by keccak256(memo_ref) so a given invoice can only
      // be paid once (vault guards invoicePaid[invoiceId]).
      ...
      const invoiceId = keccak256(toHex(invoice.memo_ref))
      const { fee, net } = platformFee(usdcAmount)

      // 1. Approve the vault to spend exactly this amount.
      ...
      await fetch(`${API}/invoices/ref/${invoice.memo_ref}/pay`, {
        method:  'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ txHash: hash, payerAddress: address, usdcAmount, platformFee: fee, netAmount: net }),
      }).catch(() => {})
  ...
  async function handlePayWithCircle() {
    ...
    try {
      ...
      const { fee, net } = platformFee(usdcAmount)
      const outcome = await payWithCircle(target, usdcAmount, invoice.memo_ref)
      ...
      await fetch(`${API}/invoices/ref/${invoice.memo_ref}/pay`, {
        ...
        body: JSON.stringify({ txHash: hash ?? null, payerAddress: null, usdcAmount, platformFee: fee, netAmount: net }),
      }).catch(() => {})
      await createPayment.mutateAsync({
        recipientAddress: invoice.creator_address,
  ...
  return (
    <div className="mx-auto max-w-lg">
      <div className="rounded-2xl border border-app-border bg-app-surface p-6">
        ...
        {/* Fee breakdown shown before payment so the payer sees exactly what
            they pay and what the creator receives (0.1% platform fee). */}
        {!alreadyPaid && !isCancelled && usdcAmount > 0 && (
          <div className="mb-5 rounded-xl border border-app-border bg-app-surface p-4 text-xs">
            <div className="space-y-1.5 text-app-muted">
              <div className="flex justify-between">
                <span>You pay</span>
                <span className="font-mono text-app-text">{formatAmount(usdcAmount, 6)} USDC</span>
              </div>
              <div className="flex justify-between">
                <span>Platform fee ({PLATFORM_FEE_LABEL})</span>
                <span className="font-mono text-app-text">{formatAmount(platformFee(usdcAmount).fee, 6)} USDC</span>
              </div>
              <div className="flex justify-between border-t border-app-border pt-1.5">
                <span>Creator receives</span>
                <span className="font-mono text-app-text">{formatAmount(platformFee(usdcAmount).net, 6)} USDC</span>
              </div>
            </div>
          </div>
        )}
