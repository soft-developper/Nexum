...
export default function OfferDetailPage() {
  ...
  // Live FX for float offers. MUST run unconditionally (before any early
  // return) to keep hook order stable - offer may be null on first render,
  // so guard the pair string; useRate tolerates a non-matching pair.
  ...
  // While a float offer is open the fiat tracks the live rate, priced on the
  // NET usdc (after platform fee) so the buyer pays fiat only for the USDC they
  // actually receive - matching how local_amount was computed at creation.
  const isFloat = (offer.order_type ?? 'market') === 'float'
  const displayLocalAmount =
    isFloat && offer.status === 'open' && liveFx?.rate
      ? platformFee(Number(offer.usdc_amount)).net * Number(liveFx.rate)
    ...
    <div>
      ...
      <div className={`grid gap-4 ${showChat ? 'lg:grid-cols-3' : 'lg:grid-cols-2'}`}>
        ...
        <div className="rounded-xl border border-app-border bg-app-surface p-5">
          ...
          <div className="mt-2 flex justify-between text-xs">
            <span className="text-app-muted">Platform fee ({PLATFORM_FEE_LABEL})</span>
            <span className="font-mono text-app-text">
              {platformFee(Number(offer.usdc_amount)).fee.toFixed(4)} USDC
            </span>
          </div>
          <div className="mt-2 flex justify-between text-xs">
            <span className="text-app-muted">Buyer receives</span>
            <span className="font-mono text-app-text">
              {platformFee(Number(offer.usdc_amount)).net.toFixed(4)} USDC
            </span>
          </div>
