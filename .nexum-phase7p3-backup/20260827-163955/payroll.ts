...
// GET /payroll/recipients/export?wallet=0x&from=<ts>&to=<ts>
// Bulk per-recipient rows across all of a wallet's batches in a date range,
// enriched with the batch name + chain. Powers the payroll dashboard CSV export.
// Named columns only (payroll_recipients DDL column order differs from the
// positional normRecipient mapping, so SELECT * + index access is unsafe here).
router.get('/recipients/export', async (req, res) => {
  ...
  try {
    const rows = parseRows(await db.run(sql`
      SELECT
        ...
        r.memo_ref       AS memo_ref,
        r.created_at     AS created_at,
        b.created_at     AS batch_created_at
      FROM payroll_recipients r
      JOIN payroll_batches b ON b.id = r.batch_id
    ...
    const recipients = rows.map((row: any) => ({
      ...
      batch_created_at: Number(row.batch_created_at),
    }))
