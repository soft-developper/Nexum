...
import {
  ...
  cancelPlatform,
  FeeProtocol,
  withdrawFeesPlatform,
  readFeeSummary,
  readTotalFeesAvailable,
...
router.get('/overview', requirePermission(PERMISSIONS.VIEW_DASHBOARD), async (_req, res) => {
  try {
    ...
    // Bridge completed, invoices paid, payroll sent - platform-wide, to match
    // the per-user weekly volume which now tracks all five protocols.
    const bridgeVolRows = await db.run(
      sql`SELECT SUM(amount) as vol FROM bridge_transfers WHERE status = 'completed'`)
    const bvr = parseRows(bridgeVolRows)
    const bridgeVol = Number(bvr[0]?.vol ?? bvr[0]?.[0] ?? 0)

    const invVolRows = await db.run(
      sql`SELECT SUM(COALESCE(usdc_amount, amount)) as vol FROM invoices WHERE status = 'paid'`)
    const ivr = parseRows(invVolRows)
    const invVol = Number(ivr[0]?.vol ?? ivr[0]?.[0] ?? 0)

    const payVolRows = await db.run(
      sql`SELECT SUM(total_amount) as vol FROM payroll_batches WHERE status IN ('completed', 'partial')`)
    const pyr = parseRows(payVolRows)
    const payVol = Number(pyr[0]?.vol ?? pyr[0]?.[0] ?? 0)

    const totalVolume = txs.reduce((s, t) => s + t.usdVol, 0)
      + p2pVol + bridgeVol + invVol + payVol
...
// ══════════════════════════════════════════════════════════
// GET /admin/manage/ramps  - fiat on/off-ramp analytics.
//
// Surfaces, for admins, the same trading reality users see on their own
// dashboards, aggregated platform-wide:
//   - KYC'd users (Bridge + any future provider), status + funnel
//   - Volume: on-ramp (deposits delivered) + off-ramp (payouts sent), USD
//   - Per-currency / corridor breakdown
//   - Health signals: pending deposits, failed/returned drains, rejected KYC
//   - Provider breakdown (future-proof for multi-provider)
//
// USD conversion reuses the overview convention (live rates, USDC/USD == 1).
// Deposits/drains store amount as TEXT; a terminal 'payment_processed' row is
...
router.get('/ramps', requirePermission(PERMISSIONS.VIEW_RAMPS), async (_req, res) => {
  try {
    ...
    function toUSD(amount: number, currency: string): number {
      ...
      if (c === 'EURC' || c === 'EUR') return amount * (rates['EURC/USDC'] ? 1 / rates['EURC/USDC'] : 1.09)
      const rate = rates[`${c}/USDC`]
      return rate && rate > 0 ? amount / rate : 0
    }
    ...
    const customers = custRows.map((r: any) => ({
      ...
      provider:     r.provider ?? r[1] ?? 'bridgexyz',
      type:         r.customer_type ?? r[2] ?? 'individual',
      kycStatus:    r.kyc_status ?? r[3] ?? 'not_started',
      tosStatus:    r.tos_status ?? r[4] ?? 'pending',
      createdAt:    num(r.created_at ?? r[5]),
      username:     r.username ?? r[6] ?? null,
      email:        r.email ?? r[7] ?? null,
    }))

    // KYC funnel counts
    const funnel = { total: customers.length, approved: 0, pending: 0, rejected: 0, notStarted: 0 }
    for (const c of customers) {
      if (c.kycStatus === 'approved') funnel.approved++
      else if (c.kycStatus === 'rejected') funnel.rejected++
      else if (c.kycStatus === 'not_started') funnel.notStarted++
      else funnel.pending++
    }
    const byType = {
    ...
    // ---- On-ramp volume (settled deposits) ----------------------------------
    const depRows = parseRows(await db.run(sql`
      SELECT currency, amount, event_type, created_at
        FROM ramp_deposit_events
       WHERE event_type = 'payment_processed'`))
    // ---- Off-ramp volume (settled payouts) ----------------------------------
    const drainRows = parseRows(await db.run(sql`
      SELECT currency, amount, state, created_at
        FROM ramp_drain_events
       WHERE state = 'payment_processed'`))

    function windowSum(rows: any[], ccyKey: string, amtKey: string, ccyIdx: number, amtIdx: number, tsKey: string, tsIdx: number) {
      const acc = { all: 0, week: 0, day: 0, count: 0 }
      for (const r of rows) {
        const ccy = r[ccyKey] ?? r[ccyIdx] ?? 'usd'
        const amt = toUSD(num(r[amtKey] ?? r[amtIdx]), ccy)
        const ts  = num(r[tsKey] ?? r[tsIdx])
        acc.all += amt; acc.count++
        if (ts > now - week) acc.week += amt
        if (ts > now - day)  acc.day  += amt
      }
      return acc
    }
    const onramp  = windowSum(depRows,   'currency', 'amount', 0, 1, 'created_at', 3)
    const offramp = windowSum(drainRows, 'currency', 'amount', 0, 1, 'created_at', 3)

    // ---- Per-currency / corridor breakdown ----------------------------------
    const corridorMap: Record<string, { currency: string; direction: string; volume: number; count: number }> = {}
    const addCorridor = (currency: string, direction: string, usd: number) => {
      const key = `${direction}:${(currency || 'usd').toLowerCase()}`
      if (!corridorMap[key]) corridorMap[key] = { currency: (currency || 'usd').toUpperCase(), direction, volume: 0, count: 0 }
      corridorMap[key].volume += usd; corridorMap[key].count++
    }
    for (const r of depRows)   addCorridor(r.currency ?? r[0] ?? 'usd', 'onramp',  toUSD(num(r.amount ?? r[1]), r.currency ?? r[0] ?? 'usd'))
    for (const r of drainRows) addCorridor(r.currency ?? r[0] ?? 'usd', 'offramp', toUSD(num(r.amount ?? r[1]), r.currency ?? r[0] ?? 'usd'))
    const corridors = Object.values(corridorMap).sort((a, b) => b.volume - a.volume)

    // ---- Provider breakdown (future-proof) ----------------------------------
    const providerMap: Record<string, { provider: string; customers: number }> = {}
    for (const c of customers) {
      if (!providerMap[c.provider]) providerMap[c.provider] = { provider: c.provider, customers: 0 }
      providerMap[c.provider].customers++
    }
    const providers = Object.values(providerMap)

    // ---- Health signals ------------------------------------------------------
    const pendingDepRows = parseRows(await db.run(sql`
      SELECT COUNT(*) AS c FROM ramp_deposit_events
       WHERE event_type IN ('funds_received','payment_submitted')`))
    const failedDrainRows = parseRows(await db.run(sql`
      SELECT COUNT(*) AS c FROM ramp_drain_events
       WHERE state IN ('undeliverable','returned','refunded','error','canceled')`))
    const health = {
      pendingDeposits: num(pendingDepRows[0]?.c ?? pendingDepRows[0]?.[0]),
      failedOrReturnedDrains: num(failedDrainRows[0]?.c ?? failedDrainRows[0]?.[0]),
      rejectedKyc: funnel.rejected,
    }

    res.json({
      funnel, byType, providers, health, corridors,
      volume: {
        onramp:  { usd: onramp.all,  week: onramp.week,  day: onramp.day,  count: onramp.count },
        offramp: { usd: offramp.all, week: offramp.week, day: offramp.day, count: offramp.count },
        totalUsd: onramp.all + offramp.all,
      },
      customers: customers.slice(0, 200), // cap the table payload
    })
  } catch (err: any) {
    console.error('[admin/ramps] error:', err?.message)
    res.status(500).json({ error: err?.message ?? 'Failed to load ramp analytics' })
  }
})

// ══════════════════════════════════════════════════════════
// PLATFORM FEES (SUPER-ADMIN ONLY, never a grantable permission)
// ══════════════════════════════════════════════════════════
// Fee viewing AND withdrawal are gated to the main admin (super_admin) by a
// hard role check - NOT by requirePermission - so no sub-admin can be granted
// access to the fee treasury via the permissions system. Kept deliberately
// separate from MANAGE_TREASURY (which stays for other treasury features).

// Hard super-admin gate for the fee endpoints below.
function requireSuperAdmin(req: any, res: any, next: any) {
  const admin = req.admin
  if (!admin) return res.status(401).json({ error: 'Not authenticated' })
  if (admin.role !== 'super_admin') {
    return res.status(403).json({
      error: 'Fee treasury is restricted to the main admin.',
...
// USDC has 6 decimals. Base units (bigint on-chain) -> human USDC number.
const USDC_DECIMALS = 6
const USDC_UNIT = 1_000_000n
function baseToUsdc(base: bigint): number {
  // Keep 6-dp precision without float drift on the integer part.
  const whole = base / USDC_UNIT
  const frac  = base % USDC_UNIT
  return Number(whole) + Number(frac) / 1e6
}
function usdcToBase(amount: number): bigint {
  // amount is human USDC (e.g. 12.5). Round to 6 dp then to base units.
...
router.get('/fees', requireSuperAdmin, async (_req, res) => {
  try {
    const protocols = await Promise.all(
      FEE_PROTOCOLS.map(async (p) => {
        const s = await readFeeSummary(p.protocol)
        return {
          key:       p.key,
          label:     p.label,
          accrued:   { base: s.accrued.toString(),   usdc: baseToUsdc(s.accrued)   },
          withdrawn: { base: s.withdrawn.toString(),  usdc: baseToUsdc(s.withdrawn) },
          available: { base: s.available.toString(),  usdc: baseToUsdc(s.available) },
        }
      })
    )
    const totalBase = await readTotalFeesAvailable()
    res.json({
      decimals: USDC_DECIMALS,
      protocols,
      totalAvailable: { base: totalBase.toString(), usdc: baseToUsdc(totalBase) },
    })
  } catch (err: any) {
    res.status(500).json({ error: err.message })
  }
...
// POST /admin/manage/fees/withdraw - withdraw collected fees for one protocol.
// Body: { protocol: 'p2p'|'invoice', to: '0x..', amount: <human USDC number> }
// On-chain withdrawFees is itself bounded to feesAvailable(protocol), so this
// can never touch user escrow even if the amount passed is wrong.
router.post('/fees/withdraw', requireSuperAdmin, async (req: any, res) => {
  const admin = req.admin
  const { protocol, to, amount } = req.body ?? {}

  const entry = FEE_PROTOCOLS.find((p) => p.key === protocol)
  if (!entry)                return res.status(400).json({ error: 'Unknown protocol' })
  if (!to || !isAddress(to)) return res.status(400).json({ error: 'Valid recipient address required' })
  if (typeof amount !== 'number' || !Number.isFinite(amount) || amount <= 0) {
    return res.status(400).json({ error: 'Valid amount (USDC) required' })
  }

  try {
    const amountBase = usdcToBase(amount)

    // Guard in the API too (clear error) though the contract also enforces it.
    const summary = await readFeeSummary(entry.protocol)
    if (amountBase > summary.available) {
      return res.status(400).json({
        error: 'Amount exceeds available fees for this protocol',
        available: { base: summary.available.toString(), usdc: baseToUsdc(summary.available) },
      })
    }

    const hash = await withdrawFeesPlatform(entry.protocol, to as `0x${string}`, amountBase)

    await logAction(
      admin.id, admin.username, 'withdraw_platform_fees', 'fees', entry.key,
      `Withdrew ${amount} USDC of ${entry.label} fees to ${to}, tx ${hash.slice(0,14)}`,
      req.ip,
    ).catch(() => {})

    res.json({ success: true, txHash: hash, protocol: entry.key, amount, to })
  } catch (err: any) {
    res.status(500).json({ error: err.message })
  }
})
